...
ggplot(data = abalone_clean, # dataset should be clean
       aes(x = mean_ph, # x-axis should be predictor, y-axis should be response 
           y = change_in_area)) + 
  # first layer: points to represent average change in shall surface area for each pair of tanks
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
**tank B1B, where the pH tank failed**  
...
**2.98 ± 0.76 (SE(. With each increase in pH, the model predicts on increase of 2.98 ± 0.76 (SE) in change in shell surface area per day (mm^2^ d^-1^).**  

What is the intercept estimate (with standard error?)  

**-18.5 ± 6.16 (SE). When pH = 0, the model predicts that abalone change in shell surface area per day is -18.5 ± 6.16 (SE). (Note to self: this is not necessarily biologically realistic, but is something the model generates as an estimate).**
...
**x: predictor variable values for (ph)**  

**predicted:model predictions for change in shall surface area per day.**  

**std.error: standard error, precision of the prediction**

**conf.low and conf.high" range for the 95% CI around model prediction.**
